/**
	...
	\todo Gracefully handle data sets without numeric or character variables. 
...
%macro smu_count_missings_per_var(input_ds=, output_ds=);
	...
	data _null_;
		set ds_contents nobs=nvars end=last;

		if last then call symput ('nvars', left(put(nvars,8.)));
	run;
	...
	data &output_ds.;
		...
		%if &nvars. > 0 %then %do;
			...
			* Get total observations using observation number instead of nobs-option 
			...
			if last then do var_iter = 1 to &nvars.; 
